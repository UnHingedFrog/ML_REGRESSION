...
# POLYNOMIAL REGRESSION (2ND DEGREE POLYNOMIAL) - LOSS FUNCTION = (ABS(X * THETA - Y))^7  i.e. seventh order
...
def pseven(e):                          # function to calulate seventh power 
    it = np.nditer([e, None])
    for a, b in it:
        b[...] = a*a*a*a*a*a*a
    return it.operands[1]
def psix(e):                            # function to calulate sixth power 
    it = np.nditer([e, None])
    for a, b in it:
        b[...] = a*a*a*a*a*a
    return it.operands[1]
...
X[:,1]=(x[:,1])
X[:,2]=np.square(x[:,1])
theta=np.zeros((1,3))                    # initial parameters of the best fit line
